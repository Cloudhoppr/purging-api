...
# Matches files to be archived, creates an absolute path for them, and returns a list of such paths
...
# Iterates through a list of files, archives them, and passes archived file paths into list
def fileArchiver(fileList: list):
    ## Iterates through file list to archive
    for filePath in fileList:
        shutil.move(filePath, r"C://Users/trivi/Desktop/nedata/backup")
        ### If file no longer exists, append a confirmation message to the list
        if not os.path.exists(filePath):
            archiveConfirmations.append(f"{filePath} archived")
...
# Archive files at the given directory in the URL on threshold date
...
def main(sourceFolder):
    try:
        ...
        if validity is True:
            ## Returns list of files to be archived
            archiveFilesList = findMatchedFiles(baseFolder=sourceFolder)
            ...
            scheduler.enterabs(thresholdDate.timestamp(), 1, fileArchiver, argument=(archiveFilesList,))
            scheduler.run()
